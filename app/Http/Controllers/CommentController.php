...
class CommentController extends Controller
{
    ...
    function getCommentByVendor($id)
    {
        ...
        $commentsvd = DB::select("select u.name, c.content from user as u, comment_vendor as c, vendors as v where v.id = $id and u.id= c.user_id and c.vendor_id = v.id");
        return json_encode($commentsvd);
    }
    function getCommentByProduct($id)
    {

        $commentsprd = DB::select("select u.name, c.content from user as u, comment as c, products as p where p.id = $id and u.id= c.user_id and c.product_id = p.id");
